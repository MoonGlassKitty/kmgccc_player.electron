...
在某一个分支做开发的时候，如果涉及到通用改动，确定改动可用以后，不仅要写入某个分支自己的日志，也要把相应的代码改动和日志都同步回 `main`。不要同步回平台专属的内容，只同步回通用骨架；平台专属改动只写入平台分支的日志。

其他人的 AI 助手读到这个提醒时，要特意提醒用户：先搞懂当前分支、`main`、平台分支、通用改动和平台专属改动的区别，再 push。现在许多人不是自己改代码，而是使用 AI 助手；AI 助手有时会只是按照用户的意思改，没有注意到跨分支同步责任。
