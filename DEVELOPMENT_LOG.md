...
- 歌词设置默认值修正：`storedNumber` 不再把缺失的 localStorage 键通过 `Number(null)` 误判为 0，避免首次启动把所有数字设置夹到范围最小值；窗口/全屏歌词字号、翻译字号、字重、切行参数和 Lookahead 默认统一为各自范围中值，并一次性迁移旧逻辑写入的最小值。
- BK 点缀暂停扭动修正：暂停状态切换会触发背景组件重渲染，旧实现每次渲染都重新读取全局时间并改写负动画延迟，导致进入 paused 前相位瞬间跳动；现将全局相位采样固定在背景节点挂载时，普通播放/暂停重渲染不再改变动画 delay。
- 皮肤切换歌词颜色重采样：窗口播放和全屏播放每次切换皮肤都会重新读取当前背景颜色并刷新歌词色，不再只依赖曲目/主题变化；BK 背景按皮肤重建颜色发布链路，异步窗口采样增加版本校验，快速连续切换时旧皮肤结果不会覆盖新皮肤。
- 全屏渐变/Apple 歌词保守配色：封面渐变模糊与 Apple 风格按采样背景亮度在低透明度亮白与灰黑两套基色间选择，并仅混入少量经过降饱和处理的背景色相；AMLL A/B 阶段使用相同颜色，避免背景变化时歌词色跳动。
- CloudMusic 外部节拍音频 URL 兜底：当 CloudMusic webdata 里的历史播放 URL 过期返回 403、且本地 `.uc` 缓存不可用时，主进程会按 `neteaseSongId` 重新请求网易云 `song/enhance/player/url`，获取新的真实 mp3/flac URL 后下载到 `cloudmusic-audio-cache`，供封面 BPM 分析使用。
