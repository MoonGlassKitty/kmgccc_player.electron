...
- CloudMusic 外部封面节奏兜底：封面 BPM 自动分析依赖本地音频 `sourceUrl` 解码，外部 CloudMusic 只能提供 SMTC 元数据/进度，拿不到 PCM 时不能自动算真实 BPM。renderer 现在改用 `displayTrack` 和外部 `playbackTime` 驱动封面节奏；外部曲目 id 改为稳定的来源/歌名/歌手 key，避免 duration 补全后丢失手动 BPM；无音频文件时优先使用已保存/手动 BPM，否则用默认 72 BPM 先让封面节奏可用。`npm run typecheck` 与 `npm run build` 已通过。
