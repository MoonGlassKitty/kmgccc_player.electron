...
- macOS 外部播放源入口改造：Swift 侧栏不再暴露 Apple Music 段，改为“本地 / 第三方音乐软件”两段，外部段旁边增加下拉菜单，可选择“第三方音乐软件 / 其他源 / 自动检测”。新增 `ExternalPlaybackSourceMode` 并持久化到 `AppSettings`；`SystemNowPlayingProvider` 根据该模式过滤 MediaRemote owner bundle：第三方模式排除浏览器，其他源只接收浏览器，自动检测全部接收。旧的 `.appleMusic` activeSource 会在启动时迁移到 `.systemNowPlaying + thirdParty`。由于当前机器没有完整 Xcode，仅执行了 changed Swift files 的 `swiftc -parse` 语法检查。
