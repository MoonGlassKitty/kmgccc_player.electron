...
- macOS 外部播放源保护条件修正：发现 renderer 仍保留旧的 `systemPlatform !== 'win32'` 自动回退逻辑，导致 macOS 点外部源后马上被切回本地源并显示“未连接”；已改为只在非 `darwin` 平台回退。main 工作树 `npm run typecheck` 已通过；mac worktree 未安装依赖，未重复运行。
- 外部源封面/歌词/跳转补齐：MediaRemote snapshot 新增封面字段并按曲目 identity 缓存 artworkData，renderer 在外部曲目变化时复用现有歌词/封面查找 IPC 异步补全 `displayTrack`，窗口歌词和全屏歌词可读取外部源歌词；外部源 seek 改为按秒转 microseconds 调用 mediaremote-adapter `seek`，有有效 duration 时暴露 `canSeek=true`。当前 Edge 直播源底层会拒绝 seek，普通音乐 App 需以实际播放器支持情况为准。main 工作树 `npm run typecheck` 与 `npm run build` 已通过；mac worktree 未安装依赖，未重复运行。
- 外部源高清封面与歌词源顺序修正：MediaRemote 返回的 artworkData 经常是低清缩略图，外部源元数据补全不再因已有 MediaRemote 封面而跳过网络封面搜索，改为优先使用网易云/QQ 等查到的高清封面，MediaRemote 小图只做兜底；歌词自动查找顺序改为网易云、QQ、本地资料库已缓存歌词，再走 AMLL/LDDC/LRCLIB 兜底。main 工作树 `npm run typecheck` 与 `npm run build` 已通过；mac worktree 待本次冲突解决后复验。
- 外部源歌词稳定性与翻译修复：外部播放 snapshot 每 700ms 刷新进度时不再重建 `externalDisplayTrack`，避免 AMLL/歌词组件误判曲目变化并重复入场动画；网易云 yrc/lrc 与 tlyric、本地缓存 synced/plain 歌词会在主进程合成为 TTML translation span，避免渲染端只读取 `syncedLyrics` 时丢失翻译。main 工作树 `npm run typecheck` 与 `npm run build` 已通过；mac worktree 待本次冲突解决后复验。
