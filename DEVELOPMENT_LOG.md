...
- BK 点缀元素运动相位连续性补录：对齐 main 代码提交 `75bb8872`，背景画面切换会重建点缀节点，旧实现每次都复用固定负延迟，导致漂移和旋转周期性跳回同一初始位置；现按全局时间折算漂移双向周期和旋转周期的负延迟，节点重建后继续当前运动相位。
- 歌词设置默认值修正补录：对齐 main 代码提交 `eb338be8`，`storedNumber` 不再把缺失的 localStorage 键通过 `Number(null)` 误判为 0，避免首次启动把所有数字设置夹到范围最小值；窗口/全屏歌词字号、翻译字号、字重、切行参数和 Lookahead 默认统一为各自范围中值，并一次性迁移旧逻辑写入的最小值。
- BK 点缀暂停扭动修正补录：对齐 main 代码提交 `6ed9919a`，暂停状态切换会触发背景组件重渲染，旧实现每次渲染都重新读取全局时间并改写负动画延迟，导致进入 paused 前相位瞬间跳动；现将全局相位采样固定在背景节点挂载时，普通播放/暂停重渲染不再改变动画 delay。
- 皮肤切换歌词颜色重采样补录：对齐 main 代码提交 `6ed9919a`，窗口播放和全屏播放每次切换皮肤都会重新读取当前背景颜色并刷新歌词色，不再只依赖曲目/主题变化；BK 背景按皮肤重建颜色发布链路，异步窗口采样增加版本校验，快速连续切换时旧皮肤结果不会覆盖新皮肤。
- 全屏渐变/Apple 歌词保守配色补录：对齐 main 代码提交 `6ed9919a`，封面渐变模糊与 Apple 风格按采样背景亮度在低透明度亮白与灰黑两套基色间选择，并仅混入少量经过降饱和处理的背景色相；AMLL A/B 阶段使用相同颜色，避免背景变化时歌词色跳动。
...
- 分支结构与 README 重写：`main` 定位为 Swift 参考源码 + Electron 主体的完整基线，`win/mac/linux` 定位为平台专用 Electron 开发分支；main README 更新为分支说明、工程入口、Electron 外部播放状态和推荐工作流。
...
- 播放会话状态记忆：新增 `playbackSession.v1` 持久化当前曲目、播放队列、播放进度、歌词侧栏开关/宽度、音量、随机状态、播放源和页面；启动加载真实资料库后按曲目 ID 一次性恢复队列与进度，恢复后保持暂停，不再总是回到资料库第一首或默认关闭歌词栏。`npm run typecheck` 已通过。
...
- 平台分支通用外部播放快照骨架回灌：参考 `mgkccc-mac` 的外部播放封面/歌词/节拍来源同步提交（如 `f8d54b76`、`91bf26e0`、`5019ac53`）和 `mgkccc-win` 的 CloudMusic/外部歌词/外部 BPM 通路提交（如 `f42c8fbb`、`2765bb43`、`645af2ef`），在 main 的 `ExternalPlaybackSnapshot` 类型中补 `artworkUrl`、`lyricsText`、`syncedLyrics`、`audioSourceUrl`、`beatSourceUrl`、`neteaseSongId` 等通用字段；renderer 生成外部播放临时曲目时直接消费这些字段，封面、歌词和艺术封面 BPM 分析可等待平台分支填入具体来源。`npm run typecheck` 已通过。
- 平台设备检测接口占位回灌：参考 `mgkccc-mac` 的蓝牙磁带音箱检测提交（如 `fafec7b7`、`33a8fbd3`、`3f8fd033`）和 `mgkccc-win` 的实体磁带检测/自动皮肤切换提交（如 `9ac342fa`、`51e22fdc`、`49415a75`），在 main 增加 `getTapeDevicePresence` preload/IPC/type 占位，默认返回未连接，后续各平台只需填入自己的设备枚举规则；同时补充 `node_modules/` 忽略规则，避免本地安装依赖后污染 Git 状态。`npm run typecheck` 已通过。
