...
- CloudMusic 本地歌词 id 桥接：清理 renderer 侧多余的 `lookupTrackMetadata` IPC/cache，把外部曲目补全收敛到主进程 snapshot。主进程现在优先读取 `AppData/Local/NetEase/CloudMusic/webdata/file/{queue,history,playingList}`，用 SMTC 的 title/artist/album/duration 匹配 CloudMusic 本地歌曲 id、专辑和封面，再把 neteaseSongId 挂到 external snapshot；`fetchLyrics` 即使未收到 id，也会用同一套本地缓存匹配后再查歌词。网易云本地 `webdata/lyric/{id}` 缓存也作为 `fetchNetEaseLyrics` 的优先来源。实测 `he is a ghost / 虻瀬` 在公开搜索失败时，可从 CloudMusic history 匹配到 `2053218427`，并用该 id 获取歌词。`npm run typecheck` 与 `npm run build` 已通过。
