...
- CloudMusic 外部歌词失败缓存重试：实测 `w/o U 2023 / r-906` 可从本地 CloudMusic webdata 匹配到 `2046776950`，网易云歌词接口也返回 LRC；为避免 renderer 某次 empty/failed/loading 结果按同一外部 key 长期缓存，外部歌词缓存新增 `updatedAt` 并对失败/空/超时 loading 状态 8 秒后自动重试。`npm run typecheck` 与 `npm run build` 已通过。
